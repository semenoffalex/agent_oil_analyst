# 13 — Competence playbook and Safety nets

**What to build:** The Analyst role lives on `identity.md` plus an instruction skill (not BIBLE). Out-of-competence pins (weather, Python, uranium) refuse without invented numbers; the host does not block tools. Route lists exist only as playbook hints. Live-model stubs (Competence dictionary, restore-after-Drop, heading-rank, Forecast-verb override of classify) are gone. Timeout / 500 / empty completion may use Safety nets.
